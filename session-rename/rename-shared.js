...
(function (root, factory) {
  ...
  function looksLikeAutoTitle(title, firstUserText) {
    ...
    if (first.startsWith(current) || current.startsWith(first)) return true;
    // 自动标题的开头一段保持原文（打码/标点改写在后段），长标题按头部 40 字前缀比对
    if (current.length > 40 && first.startsWith(current.slice(0, 40))) return true;
