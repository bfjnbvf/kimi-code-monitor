...
function render() {
  ...
  const bodyEl = root.querySelector('.ksb-walk-body');
  bodyEl.textContent = '';
  if (step.bodyHTML) {
    // bodyHTML 仅限调用方传入的静态标记，禁止注入用户输入；优先使用 bodyText。
    bodyEl.innerHTML = step.bodyHTML;
  } else if (step.bodyText) {
    bodyEl.textContent = step.bodyText;
  }
