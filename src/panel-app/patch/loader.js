...
(() => {
  ...
  try {
    ...
    if (urlOrigin) {
      window.__vibepalKapOrigin = urlOrigin;
      try { localStorage.setItem('vibepal.kapOrigin', urlOrigin); } catch (e) { /* 忽略 */ }
    }
    if (!window.__vibepalKapOrigin) {
      const spa = sessionStorage.getItem('kimi-desktop-server-origin');
      if (spa) window.__vibepalKapOrigin = spa;
    }
    if (!window.__vibepalKapOrigin) {
      const cached = localStorage.getItem('vibepal.kapOrigin');
      if (cached) window.__vibepalKapOrigin = cached;
    }
  } catch (e) { /* 忽略，走兜底 */ }
  ...
  // 时序坑：loader 比 panel-app 先跑，bridge（__vibepal）还没装好时的推送会被丢，
  // 所以等面板就绪事件补推一次。
  let lastUsageDaily = '';
  let usageFileState = '载入中';
  const pushUsageDaily = (j) => {
    window.__vibepal && window.__vibepal.push({
      v: 1, type: 'usageDaily',
  ...
  const syncUsageDaily = (force) => {
    ...
    s.src = '/vibepal/usage-daily.js?t=' + Date.now();
    s.onload = () => {
      s.remove();
      const j = window.__vibepalUsageDaily;
  ...
  window.addEventListener('vibepal:panel-ready', () => syncUsageDaily(true), { once: true });
  ...
  const syncExternal = () => {
    const s = document.createElement('script');
    s.src = '/vibepal/external.js?t=' + Date.now();
    s.onload = () => {
      s.remove();
      const j = window.__vibepalExternal;
      if (!j || typeof j !== 'object' || !window.__vibepal) return;
      const text = JSON.stringify(j);
      if (text === lastExternal) return;
      lastExternal = text;
      window.__vibepal.push({ v: 1, type: 'external', providers: j.providers });
    };
    s.onerror = () => s.remove();
    document.head.appendChild(s);
  };
  ...
  window.__vibepalAssets = {
    'rive/rive.wasm': '/vibepal/rive/rive.wasm?v=' + V,
    'rive/kimi_avatar_web-PnsTWI-X.riv': '/vibepal/rive/kimi_avatar_web-PnsTWI-X.riv?v=' + V,
    'rive/kimi_avatar_default-srYjF2HV.riv': '/vibepal/rive/kimi_avatar_default-srYjF2HV.riv?v=' + V
  };

  // 挂载点与 kimi-code-monitor 扩展一致：aside.side > .col 里 side-footer 之前
  window.__vibepalMountInto = (host) => {
  ...
  const boot = () => {
    ...
    if (!document.getElementById('vibepal-panel-css')) {
      const link = document.createElement('link');
      link.id = 'vibepal-panel-css';
      link.rel = 'stylesheet';
      link.href = '/vibepal/content.css?v=' + V;
      document.head.appendChild(link);
    }
    // Rive 运行库先于面板装配（petStart 读 globalThis.rive）
    const rive = document.createElement('script');
    rive.src = '/vibepal/rive/rive.js?v=' + V;
    rive.onload = () => {
      const app = document.createElement('script');
      app.src = '/vibepal/panel-app.js?v=' + V;
  ...
  const publishDiag = () => {
    try {
      window.__vibepalDebug = {
        ...window.__vibepalDebug,
        fileState: usageFileState,
        kapKnown: Boolean(window.__vibepalKapOrigin)
      };
    } catch (e) { /* 忽略 */ }
  };

    // 看门狗：SPA 重绘把面板卸载时挂回；顺带刷新锁位上的诊断状态行
    // （只在数据未到位、锁可见时展示；各环节状态来自 loader 与 direct.js 的 __vibepalDebug）
    setInterval(() => {
      const host = document.getElementById('ksb-panel-host');
      if (host && !host.isConnected) window.__vibepalMountInto(host);
      publishDiag();
      const status = document.getElementById('vibepal-data-status');
      if (status) {
        const d = window.__vibepalDebug || {};
        const path = location.pathname && location.pathname !== '/'
          ? (location.pathname.length > 22 ? location.pathname.slice(0, 22) + '…' : location.pathname)
          : '/';
        status.textContent = [
          '路径 ' + path,
          'kap ' + (window.__vibepalKapOrigin ? '已知' : '未知'),
          '数据 ' + usageFileState,
          'WS ' + (d.wsState || '?'),
