...
### 2.4 规则勾选框与一次性引导（实测反馈）

- 三档规则各自带勾选框（设置键 singleDayEnabled/multiDayEnabled/allEnabled）：未勾选的规则不参与判定，行置灰、天数输入停用；阈值判定见 tidy-rules.js。
- 首次「清理并解锁自动归档」成功后，面板就地弹出一次性规则说明（三条规则独立、勾选启停、24h 节奏），点击其他位置或「知道了」关闭；标记 kimiTidyGuideShown 后不再显示。
