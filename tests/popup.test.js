...
const allPopupSource = [usageSource, accountsSource, externalSource, renameSource, petsSource].join('\n');
...
test('扫描中只轮询内存进度，并显示整数百分比', () => {
  ...
  assert.match(usageSource, /正在读取本地记录 \$\{progress\}%/);
});
...
test('外部账户行：textContent 渲染无注入面，改名/移除统一及失败保留列表', () => {
  ...
  assert.doesNotMatch(allPopupSource, /innerHTML\s*=\s*[^\s'"]/);
  // 操作与 Kimi 账户统一：改名 + 移除，无「删除」字样
  assert.match(externalSource, /renameBtn\.textContent = '改名'/);
  assert.match(externalSource, /removeBtn\.textContent = '移除'/);
  ...
  assert.match(externalSource, /if \(!response\?\.ok\) throw new Error\(response\?\.error \|\| '移除失败'\)/);
  assert.match(externalSource, /renderExternalAccounts\(`移除失败：\$\{error\?\.message \|\| error\}`\)/);
  assert.match(externalSource, /catch \(error\) \{[^}]*renderExternalAccounts/s);
});

test('活跃热力图作为指标选项：固定 140 天窗口，版式与其他指标一致', () => {
...
test('Kimi 账户区块：列表与添加入口，操作走独立消息', () => {
  ...
  assert.match(accountsSource, /badge\.textContent = '当前'/);
});
